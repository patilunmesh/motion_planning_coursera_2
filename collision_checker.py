...
#completed as assignment by coursera self-driving car specialisation
#Date: 30th Nov, 2019
#student name: Unmesh Patil
    ...
    def collision_check(self, paths, obstacles):
        ...
        for i in range(len(paths)):
            ...
            for j in range(len(path[0])):
                ...
                # TOODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                # --------------------------------------------------------------
                circle_locations[:, 0] = path[0][j] + circle_offset*cos(yaw)
                circle_locations[:, 1] = path[1][j] + circle_offset*sin(yaw)
                # --------------------------------------------------------------
    ...
    def select_best_path_index(self, paths, collision_check_array, goal_state):
        ...
        for i in range(len(paths)):
            ...
            if collision_check_array[i]:
                ...
                # TOODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                # --------------------------------------------------------------
                centerline_deviation_factor = 1
                deviation = np.linalg.norm( paths[i][0][len(paths[i][0])-2] - goal_state[0] , paths[i][1][len(paths[i][0])-2] - goal_state[1])
                score = centerline_deviation_factor*deviation
                # --------------------------------------------------------------
                ...
                for j in range(len(paths)):
                    if j == i:
                        ...
                        if not collision_check_array[j]:
                            # TOODO: INSERT YOUR CODE BETWEEN THE DASHED LINES (how far away it is from other
                            # paths that are in collision.)
                            # --------------------------------------------------
                            # comment by unmesh patil
                            # Checking only immediate neighbouring paths whether they are collision free
                            if j == i +1 or j == i-1 :
                                obstacle_deviation = score*0.5 
                            score += self._weight * obstacle_deviation 
                            # --------------------------------------------------
